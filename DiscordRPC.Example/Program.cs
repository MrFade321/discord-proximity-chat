...
namespace DiscordRPC.Example
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // BasicExample();  This is what I was using before
            //  ReadyTaskExample();
            //FullClientExample();
            //Issue104();
            //IssueMultipleSets();
            ...
            Application.SetCompatibleTextRenderingDefault(false); // Set text rendering mode
            Application.Run(new Main()); // Run the form and start the application loop
        ...
        {
            // == Create the client
            var client = new DiscordRpcClient("1297861472273043517", pipe: discordPipe)
            {
            ...
            client.OnSelectVoiceChannel += (sender, msg) =>
            {
                ...
                foreach (var VoiceState in msg.Voice_States)
                {
                    Console.WriteLine($"Users -> {VoiceState.User.Username}");
                    Console.WriteLine($"Volume -> {VoiceState.Volume}");
                    Console.WriteLine($"Pan -> {VoiceState.Pan.ToString()}");

                    Pan MyPan = new Pan();
                    MyPan.Left = 0.0f;
                    MyPan.Right = 1.0f;                    
                    client.SetUserVoiceSettings(VoiceState.User.Id, MyPan, 200, false);
                }
                Console.WriteLine("<----------------------------->");
            };
            ...
            //// client.SendAuthenticateRequest();
            //Pan MyPan = new Pan();
            //MyPan.Left = 1;
            //MyPan.Right = 0;
            //client.SetUserVoiceSettings("447287594309517323", MyPan, 100, false);

            // == Set the presence
            client.SetPresence(new RichPresence()
            {
                Details = "Hacker man things",
                State = "In Game",
                Timestamps = Timestamps.FromTimeSpan(999999999),
                Buttons = new Button[]
                {
