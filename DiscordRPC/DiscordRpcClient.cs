...
namespace DiscordRPC
{
    ...
    public sealed class DiscordRpcClient : IDisposable
    {
        ...
        /// <summary>
        /// Returning our selected Channel info
        /// <para>If <see cref="AutoEvents"/> is true then this event will execute on a different thread. If it is not true however, then this event is not invoked untill <see cref="Invoke"/> and will be on the calling thread.</para>
        /// </summary>
        public event OnSelectedVoiceChannelEvent OnSelectVoiceChannel;
		...
		private void ProcessMessage(IMessage message)
        {
            ...
            switch (message.Type)
            {
                ...
                case MessageType.SelectedVoiceChannel:
                    if (OnSelectVoiceChannel != null)
                        OnSelectVoiceChannel.Invoke(this, message as SelectedVoiceChannel);
                    break;
        ...
        public void SendAuthenticateRequest()
        {
            ...
            connection.EnqueueCommand(new AuthenticateCommand() { AuthToken = "124132412313124"});
        }

        public void SetUserVoiceSettings(string UserID,Pan pan,int volume,bool mute)
        {
            if (IsDisposed)
                throw new ObjectDisposedException("Discord IPC Client");
            ...
            SetUserVoiceSetting cmd = new SetUserVoiceSetting();
            cmd.userID = UserID;
            cmd.pan = pan;
            cmd.volume = volume;    
            cmd.mute = mute;
        ...
        public void GetCurrentVoiceChannel()
        {
            if (IsDisposed)
                throw new ObjectDisposedException("Discord IPC Client");
            ...
            connection.EnqueueCommand(new GetSelectedVoiceChannel());
        }
